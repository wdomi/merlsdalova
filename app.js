/***************************************************************************
 * Merls da l'Ova – FULL app.js (Corrected Version)
 **************************************************************************/
...
async function loadIndividuals() {
  try {
    ...
    // Safety check: ensure birds is an array
    if (!Array.isArray(birds)) {
      console.error("API returned non-array data:", birds);
      birds = [];
    }
...
function renderBirds() {
  ...
  birds.filter(birdMatches).forEach(b => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <div style="display:grid; grid-template-columns:auto auto; column-gap:8px;">
      ...
      <td>
        <select 
          class="action-select"
          data-id="${b.bird_id}"
          style="padding: 4px 8px; border-radius: 6px; border: 1px solid #ccc; font-size: 12px; min-width: 130px;">
...
async function saveSelectedReports() {
  ...
  if (!latInput || !lonInput || !observerSelect) {
  ...
  for (const entry of entries) {
    ...
    if (!actionId) {
      alert("Bitte für jeden Vogel eine Aktion auswählen.");
      return;
    }
    ...
    const payload = {
      ...
      date: dateVal,
      time_manual: timeVal,
      observer: Number(observerSelect.value) // Reads from the new dropdown
    };
  ...
  visible.forEach(r => {
    ...
    .bindPopup(`
      <div>
        ...
        ${r.date}<br>
        <span style="font-size:11px;color:#c33;cursor:pointer;text-decoration:underline;" onclick="deleteObservation(${r.id})">löschen</span>
      </div>
    `)
    .addTo(latestLayer);
...
function wgs84ToLV95(lat, lon) {
  ...
  const east = 2600000 + 211455.93 * lonAux - 10938.51 * lonAux * latAux - 0.36 * lonAux * latAux ** 2 - 44.54 * lonAux ** 3;
  const north = 1200000 + 308807.95 * latAux + 3745.25 * lonAux ** 2 + 76.63 * latAux ** 2 - 194.56 * lonAux ** 2 * latAux + 119.79 * latAux ** 3;
